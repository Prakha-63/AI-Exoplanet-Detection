...
if uploaded_file is not None:
    ...
    try:
        ...
        if dl_model is not None and len(X_target) > 0:
            ...
            attention_extractor = tf.keras.Model(inputs=dl_model.inputs, outputs=[dl_model.output, dl_model.layers[-4].output])
            prob, att_w = attention_extractor.predict(test_seq, verbose=0)

            # The AttentionLayer returns (context_vector, attention_weights);
            # grab the attention weights (last element) if a list/tuple is returned
            if isinstance(att_w, (list, tuple)):
                att_w = att_w[-1]

            # Interpolate attention weights
            att_weights_1d = np.array(att_w).flatten()
